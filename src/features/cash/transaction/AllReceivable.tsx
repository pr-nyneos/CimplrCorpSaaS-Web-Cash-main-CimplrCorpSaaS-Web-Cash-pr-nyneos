...
import { ReceivableSection } from "./config";
...
const AllReceivable: React.FC = () => {
  ...
  const [editValues, setEditValues] = useState<Receivable>({} as Receivable);
    ...
    nos
      ...
      .then((response) => {
        if (response.data.success && response.data.data) {
          setData(response.data.data.receivables);
          setLoading(false);
        } else {
  ...
  const handleApprove = async () => {
    const selectedRows = table
      .getSelectedRowModel()
      .rows.map((row) => row.original.receivable_id);

    if (selectedRows.length === 0) {
      notify("Please select at least one Receivable to approve.", "warning");
      return;
    }
    ...
    if (
      ...
      notify(
        "You cannot approve a Receivable that is already approved.",
        "warning"
      );
      return;
    }
    ...
    const result = await confirm(
      `Are you sure you want to approve ${
        selectedRows.length > 0 ? selectedRows.join(", ") : "this receivable"
      }?`,
      {
        input: true,
    ...
    try {
      ...
      if (response.data.success) {
        setData((prev) =>
          prev.filter((row) => {
            if (selectedRows.includes(row.receivable_id)) {
              if (
                typeof row.status === "string" &&
        ...
        notify("Receivable approved successfully.", "success");
      } else {
  ...
  const handleDelete = async (receivable_id: string) => {
    const selectedRows = table
      .getSelectedRowModel()
      .rows.map((row) => row.original.receivable_id);
    ...
    const rowsToCheck =
      selectedRows.length > 0
        ? selectedObjs
        : [data.find((item) => item.receivable_id === receivable_id)].filter(
            Boolean
          );
    ...
    if (
      ...
      notify(
        "You cannot delete a Receivable that is already pending delete approval.",
        "warning"
      );
      return;
    }
    ...
    const result = await confirm(
      `Are you sure you want to delete ${
        selectedRows.length > 0 ? selectedRowNames.join(", ") : receivable_id
      } receivable(s)?`,
      {
        input: true,
    ...
    const payload = {
      transaction_ids: selectedRows.length > 0 ? selectedRows : [receivable_id],
    ...
    try {
      ...
      if (response.data.success) {
        setData((prev) =>
          prev.map((row) =>
            payload.transaction_ids.includes(row.receivable_id)
              ? { ...row, status: "PENDING_DELETE_APPROVAL" }
              : row
          )
        );
        notify(`${receivable_id} deleted.`, "success");
      } else {
  ...
  const handleReject = async () => {
    ...
    if (selectedRows.length === 0) {
      notify("Please select at least one Receivable to reject.", "warning");
      return;
    }
    const selectedCategoryObjs = table
      .getSelectedRowModel()
    ...
    if (
      ...
      notify(
        "You cannot reject a Receivable that is already approved.",
        "warning"
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        setData((prev) =>
          prev.map((row) =>
            selectedRows.includes(row.receivable_id)
              ? { ...row, status: "REJECTED" }
              : row
          )
        );
        notify("Receivable rejected successfully.", "success");
      } else {
  ...
  const handleEditChange = (
    ...
    value: Receivable[keyof Receivable]
  ) => {
    setEditValues((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleCancelEdit = () => {
    setIsEditing(false);
    setEditValues({} as Receivable);
  };

  const handleForwardEditToggle = async (row: Row<Receivable>) => {
    if (isEditing) {
      ...
      try {
        ...
        if (response.data.success) {
          setData((prev) =>
            prev.map((row) =>
              row.receivable_id.includes(row.receivable_id)
                ? { ...row, status: "PENDING_EDIT_APPROVAL" }
                : row
            )
          );
          setIsEditing(false);
          notify("Receivable updated successfully!", "success");
        } else {
          notify(
        ...
        notify("An error occurred while updating the receivable.", "error");
        return;
      } finally {
        setIsSaving(false);
        setEditValues({} as Receivable);
  ...
  const columns = useMemo<ColumnDef<Receivable>[]>(
    () => [
      ...
      {
        accessorKey: "old_invoice_amount",
        header: "Old Amount",
        cell: ({ getValue }) => (
          <span className="font-semibold">
            {(getValue() as number)?.toLocaleString() || "—"}
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-1">
            <button
              onClick={() => handleDelete(row.original.receivable_id)}
