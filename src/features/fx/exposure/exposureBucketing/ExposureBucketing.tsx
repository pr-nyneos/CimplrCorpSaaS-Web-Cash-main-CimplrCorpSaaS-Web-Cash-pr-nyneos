...
const ExposureBucketing = () => {
  ...
  const [editValues, setEditValues] = useState<AllExposureBucketing>(
  ...
  const getChangedFields = (original: AllExposureBucketing, edited: AllExposureBucketing) => {
    ...
    for (const key in edited) {
      const typedKey = key as keyof AllExposureBucketing;
      // Only include fields that have actual values and are different from original
      if (edited[typedKey] !== undefined && 
          edited[typedKey] !== null && 
          edited[typedKey] !== "" && 
          original[typedKey] !== edited[typedKey]) {
        (changes as any)[typedKey] = edited[typedKey];
      }
    }
    return changes;
  };
...
//   };
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <NyneOSTable<AllExposureBucketing>
        table={table}
        columns={columns}
