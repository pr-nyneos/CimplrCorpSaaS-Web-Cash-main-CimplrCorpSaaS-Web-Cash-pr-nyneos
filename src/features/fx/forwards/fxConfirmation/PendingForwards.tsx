...
const PendingForwards: React.FC = () => {
  ...
  // const [editValues, setEditValues] = useState<PendingForwards>(
  //   {} as PendingForwards
  // );
  const [data, setData] = useState<PendingForwards[]>([]);
  ...
  useEffect(() => {
    setLoading(true);
    nos
      .post<PendingForwardsResponse>(`${apiBaseUrl}/fx/forwards/entity-relevant-list`)
      .then((response) => {
  ...
  //   setIsEditing(false);
  //   setIsSaving(false);
  //   setEditValues({} as PendingForwards);
  // };
  ...
  const handleDelete = async (system_transaction_id: string, reference_id: string) => {
    ...
    try {
      const response = await nos.post<PendingForwardsResponse>(
  ...
  const columns = useMemo<ColumnDef<PendingForwards>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
...
export default PendingForwards;
