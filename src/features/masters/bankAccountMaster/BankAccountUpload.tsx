...
import { templates, bankAccountValidationConfig } from "./config.ts";
...
const UploadFile: React.FC = () => {
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    const processFile = async (file: File, fileData: UploadedFile) => {
      try {
        const validationConfig = bankAccountValidationConfig;
  ...
  for (const { blob, name } of pendingUploads) {
    ...
    try {
      const res = await nos.post<UploadBankResponse>(
        `${apiBaseUrl}/master/bankaccount/upload`,
        formData,
        {
      ...
      <div className="space-y-6">
        ...
        {files.length > 0 && (
          <UploadSummaryItem
            ...
            validationConfig={bankAccountValidationConfig}
