...
import { templates, payableReceivableValidationConfig } from "./config.ts";
...
const UploadFile: React.FC = () => {
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    const processFile = async (file: File, fileData: UploadedFile) => {
      try {
        const validationConfig = payableReceivableValidationConfig;
  ...
  const handleSetManually = async () => {
    ...
    const filesWithErrors = files.filter((file: UploadedFile) =>
      file.validationErrors ? file.validationErrors.length > 0 : false
    );
    ...
    for (const file of files as UploadedFile[]) {
      try {
        ...
        if (blob) {
          ...
          console.log(`Prepared CSV for upload (${fileName}):\n`, text);
        }
      } catch (err) {
        notify(
    ...
    // Log final CSV data for upload
    for (const { blob, name } of pendingUploads) {
      const text = await blob.text();
      console.log(`Final CSV data for upload (${name}):\n${text}`);
    }
    ...
    for (const { blob, name } of pendingUploads) {
      ...
      try {
        const res = await nos.post<UploadBankResponse>(
          `${apiBaseUrl}/master/payablereceivable/upload`,
          formData,
          {
      ...
      <div className="space-y-6">
        ...
        {files.length > 0 && (
          <UploadSummaryItem
            ...
            validationConfig={payableReceivableValidationConfig}
