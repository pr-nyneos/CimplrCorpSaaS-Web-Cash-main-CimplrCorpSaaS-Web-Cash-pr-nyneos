...
import { templates, glAccountValidationConfig } from "./config.ts";
...
const UploadFile: React.FC = () => {
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    const processFile = async (file: File, fileData: UploadedFile) => {
      try {
        const validationConfig = glAccountValidationConfig;
    ...
    for (const { blob, name } of pendingUploads) {
      ...
      try {
        const res = await nos.post<UploadBankResponse>(
          `${apiBaseUrl}/master/glaccount/upload`,
          formData,
          {
      ...
      <div className="space-y-6">
        ...
        {files.length > 0 && (
          <UploadSummaryItem
            ...
            validationConfig={glAccountValidationConfig}
