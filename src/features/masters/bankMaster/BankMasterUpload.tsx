...
import { templates,bankMasterValidationConfig} from "./config.ts";
...
const UploadFile: React.FC = () => {
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    const processFile = async (file: File, fileData: UploadedFile) => {
      try {
        const validationConfig = bankMasterValidationConfig;
...
const handleSetManually = async () => {
  ...
  // Prepare all files for upload"p
      ...
      <div className="space-y-6">
        ...
        {files.length > 0 && (
          <UploadSummaryItem
            ...
            validationConfig={bankMasterValidationConfig}
