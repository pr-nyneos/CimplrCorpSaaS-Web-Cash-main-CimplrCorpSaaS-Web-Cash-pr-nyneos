...
import { templates, entityCashValidationConfig } from "./config.ts";
...
const UploadFile: React.FC = () => {
  ...
  const handleFiles = async (fileList: FileList) => {
    ...
    const processFile = async (file: File, fileData: UploadedFile) => {
      try {
        const validationConfig = entityCashValidationConfig;
  ...
  for (const { blob, name } of pendingUploads) {
    ...
    try {
      const res = await nos.post<UploadBankResponse>(
        `${apiBaseUrl}/master/entitycash/upload`,
        formData,
        {
      ...
      <div className="space-y-6">
        ...
        {files.length > 0 && (
          <UploadSummaryItem
            ...
            validationConfig={entityCashValidationConfig}
