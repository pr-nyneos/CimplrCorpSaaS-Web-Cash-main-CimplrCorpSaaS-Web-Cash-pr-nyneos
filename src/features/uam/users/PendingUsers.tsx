...
import type AllUsers from "./AllUser.tsx";
...
const mapUser = (item: any): PendingUsers => ({
  id: item.id,
...
const PendingUsers = () => {
  ...
  const [data, setData] = useState<PendingUsers[]>([]);
  const [isEditing, ] = useState(false);
  const [editValues, ] = useState<PendingUsers>({} as PendingUsers);
  ...
  useEffect(() => {
    setLoading(true);
    nos
      .post<PendingUsersResponse>(`${apiBaseUrl}/uam/users/get-users`)
      .then((response) => {
  ...
  const handleDelete = async (id: number | string, employeeName: string) => {
    ...
    try {
      const response = await nos.post<PendingUsersResponse>(
  ...
  //   field: keyof PendingUsers,
  //   value: PendingUsers[keyof PendingUsers]
  // ) => {
  //   setEditValues((prev) => ({
  //     ...prev,
  //     [field]: value,
  ...
  // const handleEditToggle = async (row: Row<PendingUsers>) => {
  //   if (isEditing) {
  ...
  const handleApprove = async () => {
    ...
    const payload = {
      userIds: selectedRows as (string | number)[],
