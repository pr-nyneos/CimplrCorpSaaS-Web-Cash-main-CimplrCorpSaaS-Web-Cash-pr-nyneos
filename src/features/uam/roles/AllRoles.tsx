...
const mapRole = (item: any): AllRoles => ({
  id: item.id,
...
const getChangedFields = (
  ...
  const changed: Partial<AllRoles> = {};
  (Object.keys(edited) as Array<keyof AllRoles>).forEach((key) => {
    if (edited[key] !== original[key]) {
      (changed as any)[key] = edited[key];
    }
  });
  return changed;
};
...
const AllRoles = () => {
  ...
  const [editValues, setEditValues] = useState<AllRoles>({} as AllRoles);
  ...
  useEffect(() => {
    setLoading(true);
    nos
      .post<AllRolesResponse>(`${apiBaseUrl}/uam/roles/page-data`)
      .then((response) => {
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      const response = await nos.post<AllRolesResponse>(
  ...
  const columns = useMemo<ColumnDef<AllRoles>[]>(
    () => [
      {
        id: "select",
        header: ({ table }) => (
  ...
  const handleEditChange = (
    field: keyof AllRoles,
    value: AllRoles[keyof AllRoles]
  ) => {
    setEditValues((prev) => ({
      ...prev,
      [field]: value,
  ...
  const handleEditToggle = async (row: Row<AllRoles>) => {
    if (isEditing) {
      
      // const hasChanges = Object.keys(editValues).some(key => 
      //   editValues[key as keyof AllRoles] !== row.original[key as keyof AllRoles]
      // );
      ...
      // Reset any previous edit state and start fresh
